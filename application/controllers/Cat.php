...
class Cat extends CI_Controller {
	...
	private function _updateNilai($idSimulasi, $before, $after)
	{
		...
		| [2] = X   ---> Merupakan pilihan jawaban peserta (A, B, C, D, E)
		| [3] = 0   ---> Merupakan nilai dari pilihan jawaban peserta
		| [4] = F   ---> Merupakan keterangan jawaban peserta (T/F) (Sudah / Belun dijawab)
		| 
        */
		...
		//List jawaban peserta yang baru
		$y = explode(":", $after);
		$nilaiNew 	= $y[3];
	...
	public function acakSoal()
	{
		...
		$encodeKategoriSoal = json_encode($kategoriSoal);

		//Ubah data kategori soal dengan format Array Asosiatif
		$decodeKategoriSoal = json_decode($encodeKategoriSoal, true);
		
		//Acak data bank soal berdasarkan ID kategori soal
		for ($i=0; $i < count($kategoriSoal); $i++) { 
			$idKategoriSoal = $decodeKategoriSoal[$i]['id'];
			$butirSoal 		= $decodeKategoriSoal[$i]['butir_soal'];
	...
	public function index()
	{
        ...
        if ($cekToken) {
	        ...
	        //Menmpilkan data pada halaman Ujian Simulasi CAT
	...
	public function simpanJawaban($idSimulasi, $noSoal)
	{
		...
		//Jika pilihan kosong atau tudak memilih
		if ($pilihan == "") {
